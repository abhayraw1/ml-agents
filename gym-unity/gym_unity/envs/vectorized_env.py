...
from .action_flattener import ActionFlattener
...
class UnityToGymVectorized(gym.Env):
    """
    ...
    def step(self, action: List[Any]) -> GymStepResult:
        ...
        print(f'decision_step.length: {len(decision_step)} terminal_step.length: {len(terminal_step)}')
        return self._single_step(decision_step)
        # for terminal in terminal_step:
        #     obs[termina]
    ...
    def _single_step(self, info: Union[DecisionSteps, TerminalSteps]) -> GymStepResult:
        if self._allow_multiple_obs:
            ...
            for obs in visual_obs:
                visual_obs_list.append(self._preprocess_single(obs))
            default_observation = visual_obs_list
            if self._get_vec_obs_size() >= 1:
                default_observation.append(self._get_vector_obs(info))
        else:
            if self._get_n_vis_obs() >= 1:
                visual_obs = self._get_vis_obs_list(info)
                default_observation = self._preprocess_single(visual_obs[0])
            else:
